...
# Если пользователь существует - возвращает False, иначе создает и возвращает True
async def create_user(tg_id, username, referred_by):
    async with async_session() as session:
        user = await session.scalar(select(User).where(User.tg_id == tg_id))

        if not user:
            session.add(User(tg_id=tg_id, username=username, balance=15, referred_by=referred_by))
...
# Получить юзера со всеми его устройствами
...
# Получить конкретное устройство (с подгрузкой юзера, чтобы проверять права)
...
async def create_new_device(tg_id: int, os_type: str) -> Device | None:
    async with async_session() as session:
        ...
        # Красивое дефолтное имя на основе ОС
        os_names = {
            "windows": "💻 Мой Windows",
            "ios": "🍏 Мой iPhone",
            "android": "🤖 Мой Android",
